...
def get_temperature_for_month(dataset, lon_array, lat_array, sfcWind_array, valid_mask, lon, lat, datetime_obj):
    ...
    sfcWind_array (numpy array): Array of temperatures.
...
file_path_template = 'weather/wind_speed/wind_speed_{}.nc'
...
current_month = None
lon_array, lat_array, sfcWind_array, valid_mask = None, None, None, None
...
for index, row in csv_df.iterrows():
    ...
    if month != current_month:
        ...
        sfcWind_array = dataset.variables['sfcWind'][:]
        valid_mask = ~np.isnan(sfcWind_array).all(axis=0)

    try:
        print(f"Processing row {index}: {row['datetime']}, {row['lon']}, {row['lat']}")
        temperature = get_temperature_for_month(dataset, lon_array, lat_array, sfcWind_array, valid_mask, row['lon'],
                                                row['lat'], row['datetime'])  # Get the temperature
        wind_speeds.append(temperature)  # Store the temperature
    except IndexError as e:
        print(f"IndexError at row {index}: {e}")
        wind_speeds.append(np.nan)  # Append NaN if there is an error

# Add the temperature data as a new column to the CSV DataFrame
csv_df['wind_speed'] = wind_speeds

# Save the updated DataFrame with temperature data to a new CSV file

base_name = os.path.basename(csv_file_path)
name, ext = os.path.split(base_name)
new_base_name = f"{name}_windSpeed{ext}"
output_path = 'preprocessed_data/'
output_file_path = os.path.join(output_path, new_base_name)

csv_df.to_csv(output_file_path, index=False)

print(f"The new output file is saved to {output_file_path}")
