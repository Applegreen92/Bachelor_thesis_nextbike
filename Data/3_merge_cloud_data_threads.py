...
csv_file_path = 'preprocessed_data/Checked_preprocessed_data/heidelberg/temp_bike_station_data_heidelberg.csv'
...
def get_temperature_for_month(dataset, lon_array, lat_array, tas_array, valid_mask, lon, lat, datetime_obj):
    ...
    cloudcover = tas_array[time_idx, lat_idx, lon_idx]
    print(f"Extracted cloud_cover: {cloudcover}")
...
def process_month(month, df):
    ...
    tas_array = dataset.variables['clt'][:]
    ...
    for index, row in df.iterrows():
        try:
            ...
            cloudcover = get_temperature_for_month(dataset, lon_array, lat_array, tas_array, valid_mask, row['lon'],
                                                   row['lat'], row['datetime'])
            cloudcovers.append(cloudcover)
        except IndexError as e:
            print(f"IndexError at row {index}: {e}")
            cloudcovers.append(np.nan)

    df['cloud_cover'] = cloudcovers
    return df
